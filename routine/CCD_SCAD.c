/*
*  CCD_SCAD.c - Cyclic Coordinate Discent in MATLAB External Interfaces
*
***** Input ***** 
...
* 4: lambda (scalar) ... SCAD parameter
* 5: a (scalar) ... SCAD parameter
...
*
*		outMatrix = CCD_SCAD(multiplier, inMatrix)
*
...
void CCD_SCAD(double *y, double *A, double *x_ini, double *x)
{
    ...
    while(x_diff != N && t < iter_max){
        ...
        for(j = 0; j < N; j++){
            ...
            /* SCAD thresholding */
            x_old[i] = x[i];
            x[i] = sgn(tmp)*(absv(tmp)-lambda)
	      *(absv(tmp)>lambda)*(absv(tmp)<=2*lambda);
	    x[i] += sgn(tmp)*(absv(tmp)-a*lambda/(a-1))/(1-1/(a-1))
	      *(absv(tmp)>2*lambda)*(absv(tmp)<=a*lambda);
