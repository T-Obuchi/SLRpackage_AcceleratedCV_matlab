function [acve aerr]=acv_mcp(beta,Y,X,a,lambda)
%--------------------------------------------------------------------------
% acv_mcp.m: An approximate formula of cross-validation error 
% for linear regression regularized by minimax concave penalty (MCP)
%--------------------------------------------------------------------------
%
% DESCRIPTION:
...
%
% USAGE:
%    [acve aerr]=acv_mcp(beta,Y,X,a,lambda);
%
% INPUT ARGUMENTS:
...
%
% DETAILS:
%    For details of the formulation, see help of 'mcppath'
%
% REFERENCES:
...
beta_act=beta(S);
MASK=(abs(beta_act) < a*lambda);
chi_tmp=inv( X_tmp'*X_tmp+diag(MASK)/(-a) );
