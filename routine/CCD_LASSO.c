/*
*  CCD.c - Cyclic Coordinate Discent in MATLAB External Interfaces
*
***** Input ***** 
...
* 4: lambda (scalar) ... regularization parameter
* 5: theta (scalar) ... for convergence check
* 6: iter_max (scalar) ... maximum iteration
*
***** output *****
* 1: x (N+1 dim. vector) ... coefficients
* N+1-th component of x indicates the convergence;
* x[N+1] = 0 if converges, 1 otherwise
*
*****
* The calling syntax is:
*
*		outMatrix = CCD(multiplier, inMatrix)
*
...
void CCD(double *y, double *A, double *x_ini, double *x)
{
    ...
    while(x_diff != N && t < iter_max){
        ...
        for(j = 0; j < N; j++){
            ...
            /* soft thresholding */
            x_old[i] = x[i];
            x[i] = sgn(tmp)*(absv(tmp)-lambda)*(absv(tmp)>lambda);
    ...
    int nrhs, const mxArray *prhs[])
{
    ...
    iter_max = (mwSize)mxGetScalar(prhs[5]);
