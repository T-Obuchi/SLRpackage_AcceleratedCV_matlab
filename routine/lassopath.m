function [fit]=lassopath(Y,X,lambdaV,cvoption,kfold,theta,iter_max,beta_in,seed);
%--------------------------------------------------------------------------
% lassopath.m: Estimating a solution path and cross-validation error 
% for linear regression with L1 penalty (LASSO) by using lambda annealing
%--------------------------------------------------------------------------
%
% DESCRIPTION:
%    Compute a solution path of linear regression regularized by L1 penalty (LASSO).
...
%    Remark: An approximate formula of cve based on [2] is implemented, 
...
%                'approximate1': Approximate CV based on 'approximation 1' in [2];
%                'approximate2': Approximate CV based on 'approximation 2' in [2];
...
%
% DETAILS:
%    The linear regression regularized by L1 penalty is formulated as follows:
% 
%       \hat{beta}=argmin_{beta}
%           { (1/2)||Y-X*beta||_2^2 + \sum_{i}^{N}J(beta_i;lambda) }
%
%    where J denotes the L1 penalty whose functional form is
%
%       J(x;lambda)=lambda*|x|
%
%    Each estimator at given (lambda) is computed by the CCD algorithm. 
...
if nargin < 4 || isempty(cvoption)  
    cvoption='approximate2';         % Approximate CV by approximation 2 is a default setting
end
...
% cvoption check
if strcmp(cvoption,'approximate1')
    disp('Approximate cross-validation by approximation 1 conducted');
elseif  strcmp(cvoption,'approximate2')
    disp('Approximate cross-validation by approximation 2 conducted');
...
elseif  strcmp(cvoption,'none')
    ...
    disp('Please choose one of the four below and retry:') 
    disp([' ''approximate1'', ''approximate2'', ''literal'', ''none''']); 
end


% Evaluate solution path using lambda annealing
tic;
[beta,flag_conv]=CCD_LASSO_path(Y,X,lambdaV_ord,theta,iter_max,beta_in);
etime_path=toc;
...
cve=zeros(Llam,2);
if strcmp(cvoption,'approximate1')
    % Approximate CV by approximation 1
    for ilam=1:Llam
        [acve aerr]=acv_lasso(beta(:,ilam),Y,X);
        cve(ilam,1:2)=[acve aerr];
    end
    
elseif  strcmp(cvoption,'approximate2')
    % Approximate CV by approximation 2
    for ilam=1:Llam
        [acve aerr]=saacv_lasso(beta(:,ilam),Y,X);
        cve(ilam,1:2)=[acve aerr];
    end
...
elseif  strcmp(cvoption,'literal') 
    ...
    for j=1:kfold
        ...
        [beta_tmp]=CCD_LASSO_path(Y_train,X_train,lambdaV_ord,theta,iter_max,beta_in);
