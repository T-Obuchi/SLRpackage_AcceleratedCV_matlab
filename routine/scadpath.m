function [fit]=scadpath(Y,X,a,lambdaV,cvoption,kfold,theta,iter_max,beta_in,seed);
%--------------------------------------------------------------------------
% scadpath.m: Estimating a solution path and cross-validation error 
% for linear regression with smoothly clipped absolute deviation (SCAD) penalty 
% by using lambda annealing
%--------------------------------------------------------------------------
%
% DESCRIPTION:
%    Compute a solution path of linear regression 
%    regularized by smoothly clipped absolute deviation (SCAD) penalty.
...
%
% DETAILS:
%    The linear regression regularized by SCAD is formulated as follows:
% 
%       \hat{beta}=argmin_{beta}
%           { (1/2)||Y-X*beta||_2^2 + \sum_{i}^{N}J(beta_i;lambda,a) }
%
%    where J denotes the SCAD penalty whose functional form is
%
%                     { lambda*|x|                            , ( |x|      <= lambda         )
%       J(x;lambda,a)={ -(x^2-2*a*lambda*|x|+lambda^2)/(2*a-2), ( lambda   < |x| <= a*lambda )
%                     { (a+1)*lambda^2/2                      , ( a*lambda < |x|             )
%
...
tic;
[beta,flag_conv]=CCD_SCAD_path(Y,X,a,lambdaV_ord,theta,iter_max,beta_in);
etime_path=toc;
...
if strcmp(cvoption,'approximate')
    % Approximate CV
    for ilam=1:Llam
        [acve aerr]=acv_scad(beta(:,ilam),Y,X,a,lambdaV_ord(ilam));
...
elseif  strcmp(cvoption,'literal') 
    ...
    for j=1:kfold
        ...
        [beta_tmp]=CCD_SCAD_path(Y_train,X_train,a,lambdaV_ord,theta,iter_max,beta_in);
